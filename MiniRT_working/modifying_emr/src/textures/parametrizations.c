...
t_vec2s	rt_get_sphere_uv_local(t_vec4s *hitp)
{
	...
	theta = acosf(hitp->y / magnitude_vec4s_re(op_hitp));
	if (phi < 0.f)
		phi += two_pi;
	if (theta < 0.f)
		theta += (float)M_PI;
...
t_vec2s	rt_get_plane_uv_local(t_vec4s *hitp, t_vec4s tangent, t_vec4s *normal)
{
	...
	tile_factor = 0.1f;
	cross_vec4s(&bitangent, tangent, *normal);
	uv.x = dot_vec4s_re(hitp, &tangent) * tile_factor;
	uv.y = dot_vec4s_re(hitp, &bitangent) * tile_factor;
